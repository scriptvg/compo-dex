Una tarjeta inspirada en las cartas Pokémon que adapta su fondo al tipo (y tipo secundario) del Pokémon, con soporte para tamaños y un modo Mega animado.
...
Contenedor principal de la tarjeta. Renderiza un `<div>`, por lo que acepta todas sus props.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `size` | `"default" \| "sm"` | `"default"` | Tamaño de la tarjeta. `"sm"` reduce el espaciado y el relleno interno. |
| `isMega` | `boolean` | `false` | Activa el fondo animado tipo arcoíris (`rainbow-animated`) para Mega Evoluciones. |
| `type` | `PokemonBadgeType` | `"normal"` | Tipo principal del Pokémon; define el color de fondo. |
| `secondary` | `PokemonBadgeType` | `undefined` | Tipo secundario opcional; combinado con `type` produce un fondo en degradado. |
| `className` | `string` | `undefined` | Clases adicionales para extender o sobrescribir estilos. |
| `...props` | `React.ComponentProps<"div">` | - | Resto de props nativas de un `<div>`. |

### PokemonCardHeader

Cabecera de la tarjeta. Renderiza un `<div>` y acepta todas sus props.
...
Título de la tarjeta. Renderiza un `<div>` y acepta todas sus props.
