...
### Variants
Para poder mostrar todas las variantes de estilos disponibles para el componente, puedes usar el siguiente bloque de código:
...
| --- | --- | --- | --- |
| `type` | `"normal"` `"fire"` `"water"` | `"normal"`  | Tipo de Pokémon que define el color de superficie de la insignia. |
| `variant` | {/* `"default" \| "secondary" \| "destructive" \| "outline" \| "ghost" \| "link"` */} | `"default"` | Variante visual del estilo base de la insignia. |
| `asChild` | `boolean` | `false` | Si es `true`, fusiona las props en el hijo directo en lugar de renderizar un `<span>`. |
