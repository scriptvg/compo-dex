...
const pageVariants = cva("mx-auto w-full max-w-[1400px]", {
    variants: {
        variant: {
            default: "",
            sidebar: "xl:grid xl:grid-cols-[minmax(0,1fr)_14rem] xl:gap-8",
        },
    },
    defaultVariants: {
        variant: "default",
...
function PageAside({ className, ...props }: React.ComponentProps<"aside">) {
    return (
        <aside
            ...
            {...props}
            className={cn(
                "sticky top-(--header-height) hidden max-h-[calc(100svh-var(--header-height))] w-56 shrink-0 self-start overflow-y-auto py-6 xl:block",
                className,
            )}
        />
    )
